<template>
  <PageWrapper
    page-id="Animation"
    class="flex flex-col gap-2 font-terminal text-secondary"
  >
    <button
    ...
    <div class="flex flex-col gap-2" v-if="timers.history">
      ...
      <div>
        ...
        postcss.config.js README.md tailwind.config.ts tsconfig.json
        vite.config.ts robertreder.dev
      </div>
    ...
    <div class="flex gap-4" v-if="!timers.history">
      <div class="font-bold">{{ BASE_PROMPT }}</div>
      <Typing :delay="500" :speed="10">npm run dev</Typing>
    </div>

    <div v-show="timers.firstLog">Server starting...</div>
    <div v-show="timers.secondLog">
...
// After 3.5s -> start typing 'npm run dev'
// 'npm run dev' takes 1.8s.
...
onMounted(() => {
  for (const [key, value] of Object.entries(TIMES)) {
    ...
    setTimeout(
      () => (timers[key as keyof typeof timers] = !defaultValue),
      delay
    );
  }
